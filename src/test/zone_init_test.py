...
def zone_initializing_process(cur_frame, trigger, old_bg_img, mask_tuple, zone_id, of_event, zone_init_event, shutdown_event, bg_queue):
    ...
    print('ZoneInit ' + str(zone_id), 'Done.')
...
def main():
    ...
    cur_frame = cv2.imread(os.path.join(path,files[frame_idx])) # read the first image
    ...
    end_time = time.time()
    print('##### Sec/Frame:', (end_time - start_time) / frame_num, '#####')
