...
if __name__ == "__main__":
    ...
    while 1:
        ...
        if idx_frame>=10: break # files_num-1
    ...
    # for i in range(13):
    #     intance_list[i] = Renew(mask_name[i])
    #     # MyManager.register(mask_name[i][:-4], intance_list[i])
    #     print(type(intance_list[i]))
    # process_list = []
    #
    ...
    # start_time = time.time()
    # for i in range(13):
    #     print(type(intance_list[i]),id(intance_list[i]))
    #     p.apply_async(run_kernal,
    #                 args=(bgs_list[i].apply,img_undist,)) #, img_undist, flow_contour , mag, idx_frame
    # p.close()
    # p.join()
